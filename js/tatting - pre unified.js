...
$(function() {
		...
		canvas.on('mouse:move', function(o) {
				...
				if (isArcDrawing) {
					...
					arcCurvePreview=new fabric.Group(curveShapeArray,
							{
							x1: arcLine.x1,
							x2: arcLine.x2,
							y1: arcLine.y1,
				...
				if (isTeardropDrawing) {
					...
					teardropCurvePreview=new fabric.Group(curveShapeArray,
							{
							x1: teardropLine.x1,
							x2: teardropLine.x2,
							y1: teardropLine.y1,
		...
		canvas.on('mouse:up', function(o) {
				if (isArcDrawing) {
					...
					canvas.remove(arcCurvePreview);
					arcCurvePreview=new fabric.Group([]);
					drawArc(arcLine.x1, arcLine.y1, arcLine.x2, arcLine.y2);

					arcCurve=new fabric.Group(curveShapeArray,
							{
							x1: arcLine.x1,
							x2: arcLine.x2,
							y1: arcLine.y1,
				...
				if (isTeardropDrawing) {
					...
					canvas.add(teardropCurvePreview);
					canvas.renderAll();
					teardropCurvePreview=new fabric.Group([]);
